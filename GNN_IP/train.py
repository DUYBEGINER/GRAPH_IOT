"""
GraphSAGE Training for IP-based GNN
Nodes: IP addresses, Edges: Flows
Binary Classification: Benign IP vs Attack IP
...
class Trainer:
    ...
    def train(self, data, optimizer, scheduler, num_epochs, patience):
        print(f"\nTraining GraphSAGE (IP-based) on {self.device}")
...
def plot_metrics(results, save_path):
    ...
    ax.set_ylim(0, 1)
    ax.set_ylabel('Score')
    ax.set_title('Model Performance Metrics (IP-based)')
...
def main():
    ...
    print(f"Nodes (IPs): {data.num_nodes:,}, Edges: {data.num_edges:,}, Features: {data.num_features}")
